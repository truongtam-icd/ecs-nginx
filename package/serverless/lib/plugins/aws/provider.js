...
class AwsProvider {
  ...
  async dockerPushToEcr(remoteTag, options = {}) {
    ...
    try {
      ...
      throw new ServerlessError(
        `Encountered error during executing: docker pushDockerArgs ${pushDockerArgs}\nOutput of the command:\n${err.stdBuffer}`,
        'DOCKER_PUSH_ERROR'
      );
    }
  }
  ...
  memoizeeMethods({
    ...
    getOrCreateEcrRepository: d(
      async function (scanOnPush) {
        ...
        try {
          ...
          if (result.repository.repositoryUri.startsWith("localhost.localstack.cloud")) {
            repositoryUri = "localhost:4510";
          } else {
    ...
    resolveImageUriAndShaFromPath: d(
      async function ({
        ...
        const repositoryUriCustom = "localhost:4510";
        let localTag;
        let remoteTag;

        if (repositoryUri.startsWith("localhost.localstack.cloud")) {
          remoteTag = `${repositoryName}:${imageName}`;
          localTag = `${repositoryUriCustom}/${repositoryName}`;
        } else {
          remoteTag = `${repositoryUri}:${imageName}`;
          localTag = `${repositoryName}:${imageName}`;
        }
        ...
        try {
          ...
          try {
            ...
            throw new ServerlessError(
              `Encountered error during executing: docker buildDockerArgs ${buildDockerArgs.join(
                ' '
              )}\nOutput of the command:\n${err.stdBuffer}`,
              'DOCKER_BUILD_ERROR'
            );
          ...
          let remoteTagBackup = remoteTag;
          if (remoteTag.startsWith("localhost:4510:latest")) {
            const tag = remoteTag.replace(":latest", "/" + localTag).replace(":latest", "");
            localTag = tag;
            remoteTag = localTag;
          }
          ...
          if (localTag !== remoteTag) {
            tagDockerArgs = ['tag', localTag, remoteTag];
          } else {
            tagDockerArgs = ['tag', remoteTagBackup.replace("localhost:4510", repositoryName), localTag];
          }
          ...
          try {
            ...
            throw new ServerlessError(
              `Encountered error during executing: docker tagDockerArgs ${tagDockerArgs.join(
                ' '
              )}\nOutput of the command:\n${err.stdBuffer}`,
              'DOCKER_TAG_ERROR'
            );
          ...
          if (localTag.startsWith("localhost:4510")) {
            dockerPushOutput = await this.dockerPushToEcr(localTag);
          } else {
            dockerPushOutput = await this.dockerPushToEcr(remoteTag);
          }
